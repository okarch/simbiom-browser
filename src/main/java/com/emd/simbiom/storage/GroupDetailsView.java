...
/**
 * <code>GroupDetailsView</code> presents details of a storage group.
 *
 * Created: Sat Apr 27 07:35:49 2019
 *
 * @author <a href="mailto:okarch@cuba.site">Oliver Karch</a>
 * @version 1.0
 */
public class GroupDetailsView extends VelocityView {
    private String messageRowId;
    private String detailsLayout;

    private static Log log = LogFactory.getLog(GroupDetailsView.class);
    ...
    private void initCommands( Window wnd, Map context ) {
	Long lUser = (Long)context.get( "userId" );
	if( lUser == null )
	    return;
	String portletId = (String)context.get( "portletId" );
	if( portletId == null )
	    return;
	InventoryPreferences dp = InventoryPreferences.getInstance( portletId, lUser.longValue() );
	InventoryCommand[] cmds = dp.getCommands();
	for( int i = 0; i < cmds.length; i++ ) {
	    Component cmp = wnd.getFellowIfAny( cmds[i].getCommandName() );
	    if( cmp != null ) {
